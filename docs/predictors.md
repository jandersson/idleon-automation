...
where \(N_K(x_*)\) is the set of \(K\) nearest neighbours of the query and
\(d_i = \|x_* - x_i\|\).
...
predict \(y_*\) at a new point \(x_*\). Stack training outputs and the
unknown together:
...
training inputs and \(K_{*,\text{train}}\) is the \(n \times 1\) vector of
kernel values between \(x_*\) and each training input.

By the standard conditional-Gaussian formula,
\(p(y_* \mid \mathbf{y}_{\text{train}})\) is also Gaussian with:
...
The big strategic difference from KNN: \(\sigma_*^2\) tells us *how
confident the model is at this query point*. It's high when \(x_*\) is
