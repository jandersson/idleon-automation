...
| chopping | ~80%    | Working end-to-end. Default aim is **planned shots** (`planner.py`): clicks scheduled for the time-domain center of an upcoming zone crossing (compensating click latency), which keeps firing through the ~600–650 px/s speed saturation where the legacy reactive gate starves — that gate (`CHOPPING_AIM=gate`) remains the validated fallback. Gold chase with viability pricing, post-chop fire hold keyed on the zone re-roll ack, exit-on-starve banking, template-based PTS OCR (`assets/digit_templates/`) with a continuous `polls.pts_read` step function as score ground truth. Every click + full-rate polls logged to `assets/chopping.db`. Mechanics model in `docs/chopping_notes.md`; open threads in [#46](https://github.com/jandersson/idleon-automation/issues/46). |
