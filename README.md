...
| chopping | ~90%    | The most developed bot. Fully hands-off: auto-locates the overlay (environment-proof), auto-clicks Play Game, fires **planned shots** (`planner.py` — scheduled zone-center impacts with a measured error model, EV candidate ranking against an empirical death-risk curve, cross-sweep candidates), banks via starve exit or death (points bank either way). Complete PTS digit library gives a per-chop ground-truth score trail. Best 52 vs human 66. Mechanics + architecture in `docs/chopping_notes.md`; history in [#46](https://github.com/jandersson/idleon-automation/issues/46). |
| catching | ~30%    | Early bot with real instrumentation: fly + hoop detectors, flap controller, per-flap logging to `assets/catching.db` (632 flaps / 18 runs), digit capture, dynamics fitting (`catching-fit-dynamics`, `CATCHING_USE_MODEL`). Open work under [`label:minigame:catching`](https://github.com/jandersson/idleon-automation/issues?q=is%3Aissue+label%3A%22minigame%3Acatching%22): #47 tracking, #52 hoop-colour scoring, #60 anchor+biome colours, #61 apex phase-lock, #62 sim recalibration. |
